...
public class RobotPeer implements Runnable, ContestantPeer {
	...
	private static final Logger log = Logger.getLogger(RobotPeer.class.getName());	

	protected int state;

	public RobotPeer(String name) {
		log.info("RobotName: "+name);
		try {
			throw new Exception();
		} catch (Exception e) {
	...
	public void run() {
		log.info(this.name+" Robot peer running");
		setRunning(true);
		...
		try {
			...
			log.info("SYSTEM: " + getName() + " has died");
			...
			log.info("SYSTEM: Robot disabled" + msg);
			out.println("SYSTEM: Robot disabled" + msg);
		} catch (Exception e) {
			log.info(getName() + ": Exception: " + e);
			out.println(getName() + ": Exception: " + e);
			out.printStackTrace(e);
		} catch (Throwable t) {
			if (!(t instanceof ThreadDeath)) {
				log.info(getName() + ": Throwable: " + t);
	...
	public final void tick() {
		...
		//log.info(this.name+" currThread: "+Thread.currentThread()+" runthread: "+robotThreadManager.getRunThread());
		// Entering tick
		...
		synchronized (this) {
			...
			notify();
			//log.info(this.name+" set sleep to True");
			isSleeping = true;
